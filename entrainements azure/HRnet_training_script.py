...
# Image size and main task params 
...
augmented_train_gen = SegmentationGenerator(train_image_list, train_mask_list, n_classes=8, batch_size=batch_size, resize_shape=(img_height, img_width), 
                 seed = 7, crop_shape=None, horizontal_flip=True, blur = 0,
                 vertical_flip=False, brightness=False, rotation=False, zoom=False, do_ahisteq = True)
...
# Construction du modèle multi-gpu :


hrnet = seg_hrnet(batch_size, img_height, img_width, 3, 8)
hrnet.compile(optimizer='adam',
                  loss=combined_loss,
...
for epoch in range(epochs):
    model_history = hrnet.fit_generator(augmented_train_gen,
...
hrnet.load_weights('outputs/checkpoint')
hrnet.save('outputs/hrnet')